...
export default function Board({ board, lastMove, winLine, previewColor, onPlace }: BoardProps) {
  ...
  const nav = useBoardKeyboardNav({
    ...
    rows: SIZE,
    interactive,
    canPlace: (x, y) => board[idx(x, y)] === null,
  ...
  return (
    <>
      <svg
        viewBox={`0 0 ${W} ${W}`}
        className="w-full max-w-[590px] select-none rounded-lg shadow-[0_10px_30px_rgba(26,22,20,0.25)] focus:outline-none focus-visible:ring-2 focus-visible:ring-vermil"
        onMouseLeave={() => setHover(null)}
        onKeyDown={nav.onKeyDown}
        tabIndex={interactive ? 0 : -1}
        role="img"
        aria-label="오목판 — 화살표 키로 이동, Enter 또는 Space로 착수"
      >
        <defs>
