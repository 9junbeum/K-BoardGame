...
interface OthelloRoomRow {
  ...
  rules: Partial<OthelloRules> | null;
  undo: OthelloUndoState | null;
  rematch: OthelloRematchState | null;
  finished_at: string | null;
}

export default function OthelloRoom({ roomId }: { roomId: string }) {
  const router = useRouter();
  const supabase = useMemo(() => getSupabase(), []);

  const [myId, setMyId] = useState("");
  const [room, setRoom] = useState<OthelloRoomRow | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!data) {
        ...
        setRoom(data as OthelloRoomRow);
        await fetchPlayers();
      }
      setLoaded(true);
    })();
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => setRoom(payload.new as OthelloRoomRow),
      )
      .on(
        "postgres_changes",
  ...
  const rules: OthelloRules = useMemo(
    () => ({ ...DEFAULT_OTHELLO_RULES, ...(room?.rules ?? {}) }),
    [room?.rules],
  );
  const undo = room?.undo ?? null;
  const state: OthelloState = room?.state ?? createOthelloState();
  ...
  // ---------- 착수 (둘 곳이 없으면 자동으로 건너뛰므로 별도 패스 조작이 없다) ----------
  ...
  const place = useCallback(
    async (x: number, y: number) => {
      ...
      if (!r) return; // 둘 수 있는 자리만 클릭되므로 정상 진행 시 발생하지 않음
      const d = deriveOthello(r.moves);

      let update: Partial<OthelloRoomRow>;
      if (d.gameOver) {
        const blackId = players.find((p) => p.color === "b")?.player_id ?? null;
        const whiteId = players.find((p) => p.color === "w")?.player_id ?? null;
        const winner =
          d.counts.b === d.counts.w ? "draw" : d.counts.b > d.counts.w ? blackId : whiteId;
        update = {
          state: { moves: r.moves },
          status: "finished",
          winner: winner ?? "draw",
      ...
      setRoom((prev) => (prev ? ({ ...prev, ...update } as OthelloRoomRow) : prev));
      ...
      if (error || !data || data.length === 0) {
        ...
        if (fresh) setRoom(fresh as OthelloRoomRow);
      }
    },
  ...
  const requestUndo = useCallback(async () => {
    ...
    const value: OthelloUndoState = { ...(undo ?? {}), by: myId, declined: undefined };
  ...
  const acceptUndo = useCallback(async () => {
    ...
    const value: OthelloUndoState = { used: [...(undo.used ?? []), undo.by] };
    const update = {
      state: { moves },
      current_turn: players.find((p) => p.color === d.toMove)?.player_id ?? null,
      undo: value,
  ...
  const declineUndo = useCallback(async () => {
    ...
    const value: OthelloUndoState = { used: undo.used ?? [], declined: undo.by };
    setRoom((prev) => (prev ? { ...prev, undo: value } : prev));
  ...
  const acceptRematch = useCallback(async () => {
    ...
    const { data: newRoom, error } = await supabase
      ...
      .insert({
        ...
        rules: room.rules ?? DEFAULT_OTHELLO_RULES,
        undo: null,
      })
      .select("id")
      .single();
  ...
  return (
    <Shell>
      ...
      <OthelloBoard
        board={board}
        lastMove={lastMove}
        legalMoves={myTurn ? derived.legal : []}
          ...
          <div className="text-center">
            <p className="font-plex text-xs text-mud">
              {isSpectator
                ? "관전 중입니다"
                : myTurn
                  ? "당신 차례입니다 — 둘 수 있는 자리를 클릭하세요"
                  : `${opponent?.nickname ?? "상대"}의 차례입니다`}
            </p>
            {myTurn && derived.skipped && (
              <p className="mt-1 font-plex text-[11px] text-vermil">
                상대가 둘 곳이 없어 자동으로 순서가 넘어왔습니다
              </p>
            )}
