...
import CatchmindSettingsModal from "@/components/CatchmindSettingsModal";
...
import type { CatchmindRules } from "@/games/catchmind/logic";
...
export default function LobbyPage() {
  ...
  const [showCatchmindSettings, setShowCatchmindSettings] = useState(false);
  ...
  /** Supabase가 설정되어 있으면 설정 모달을, 아니면 같은 화면 대국으로 바로 이동 */
  const openGame = useCallback(
    (showModal: () => void, localPath: string) => {
      setError(null);
      if (!supabaseEnabled) {
  ...
  const startGame = useCallback(() => openGame(() => setShowSettings(true), "/room/local"), [openGame]);
  ...
  const createCatchmindRoom = useCallback(
    async (rules: CatchmindRules) => {
      const supabase = getSupabase();
      if (!supabase) return;
      setCreating(true);
      const { data, error: err } = await supabase
        .from("game_rooms")
        .insert({ game_type: "catchmind", status: "waiting", state: {}, rules })
        .select("id")
        .single();
      setCreating(false);
      if (err || !data) {
        setShowCatchmindSettings(false);
        setError("방을 만들지 못했습니다. Supabase 마이그레이션/설정을 확인해 주세요.");
        return;
      }
      router.push(`/room/${data.id}`);
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-2xl flex-col px-6 py-8">
      ...
      <section className="mt-16 text-center">
        <h1 className="text-4xl font-bold leading-tight tracking-tight sm:text-5xl">KBG</h1>
        <p className="mx-auto mt-4 max-w-md text-ink-soft">
          링크 하나로 어디서든 친구와 보드게임!
        </p>
        <div className="mt-8 flex flex-col items-center gap-3">
          <div className="grid w-full grid-cols-2 gap-3 sm:gap-4">
            <GameCard slug="omok" label="오목" onClick={startGame} disabled={creating} />
            <GameCard
              slug="yut"
              label="윷놀이"
              badge="2~4인"
              onClick={() => openGame(() => setShowYutSettings(true), "/room/local/yut")}
              disabled={creating}
            />
            <GameCard
              slug="go"
              label="바둑"
              onClick={() => openGame(() => setShowGoSettings(true), "/room/local/go")}
              disabled={creating}
            />
            <GameCard
              slug="othello"
              label="오셀로"
              onClick={() => openGame(() => setShowOthelloSettings(true), "/room/local/othello")}
              disabled={creating}
            />
            <GameCard
              slug="sagmok"
              label="사목"
              onClick={() => openGame(() => setShowSagmokSettings(true), "/room/local/sagmok")}
              disabled={creating}
            />
            <GameCard
              slug="memory"
              label="카드 뒤집기"
              onClick={() => openGame(() => setShowMemorySettings(true), "/room/local/memory")}
              disabled={creating}
            />
            <GameCard
              slug="checkers"
              label="체커"
              onClick={() => openGame(() => setShowCheckersSettings(true), "/room/local/checkers")}
              disabled={creating}
            />
            <GameCard
              slug="catchmind"
              label="캐치마인드"
              badge="2~4인"
              onClick={() => setShowCatchmindSettings(true)}
              disabled={creating || !supabaseEnabled}
            />
          </div>
          {!supabaseEnabled && (
            <p className="mt-2 max-w-sm font-plex text-xs leading-relaxed text-vermil">
              Supabase가 아직 설정되지 않아 실시간 대전은 비활성 상태입니다. 모든 게임을 같은
              화면 대국으로 즐길 수 있습니다. (.env.local 설정 후 활성화)
            </p>
          )}
      ...
      <footer className="mt-auto flex flex-col items-center gap-2 pt-12 text-center font-plex text-[10px] uppercase tracking-widest text-mud">
        <span>KBG — 링크 하나로 어디서든 친구와 보드게임</span>
      ...
      />
      <CatchmindSettingsModal
        open={showCatchmindSettings}
        creating={creating}
        onCreate={createCatchmindRoom}
        onClose={() => setShowCatchmindSettings(false)}
      />
    </main>
  );
}
